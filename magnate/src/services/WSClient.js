...
export const WSClient = () => {
    ...
    // 4102 private cancel.
    // 4103 game left.
    const closeExistingSocket = (sockcode = 4100) => {
        if (socket.current) {
    ...
    const handlePublicRoom = () => {
        if (socket.current && (socket.current.readyState === 0 || socket.current.readyState === 1)) {
            if (VERBOSE)
                console.log("DEBUG: ya hay un intento de conexión en curso.");
            return;
        }
        if (VERBOSE) {
     ...
     * @fires many many event buses TODO
     */
    const handlePrivateRoom = (roomid) => {
        if (VERBOSE) {
