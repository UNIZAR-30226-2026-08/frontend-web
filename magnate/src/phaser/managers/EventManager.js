...
export class EventManager {
    ...
    tiles;
    players;
    constructor(scene, tiles, players) {
        this.scene = scene;
        this.tiles = tiles;
    ...
    setupControlsEvents() {
        ...
        EventBus.on('start-selection-mode', (data) => {
        ...
        EventBus.on('open-property-selection-mode', (model, playerId) => {
        ...
        EventBus.on('start-tram-selection', () => {
            ...
            this.tiles.forEach(tile => {
                if (tramTileIds.includes(tile.tileConfig.id)) {
                    ...
                    tile.removeAllListeners('pointerdown');
                    tile.once('pointerdown', () => {
                        const tramConfig = tile.tileConfig;
    ...
    setupJailEvent() {
        ...
        // resultados dados
        EventBus.on('jail-dice-rolled', (res) => {
        ...
        // Pago de fianza 
        EventBus.on('execute-jail-bail-payment', (pay) => {
        ...
        EventBus.on('execute-in-jail-travel', (data) => {
            //const p = this.scene.getLocalPlayer();
            const p = this.getActivePlayer();
            if (!p)
                return;
