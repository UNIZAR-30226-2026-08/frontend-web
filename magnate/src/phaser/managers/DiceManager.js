...
export class DiceManager {
    ...
    handleDiceRoll(tiles, players, forcedValues, destinations, isMyTurn) {
        if (this.isRolling)
            return;
        this.board.hideUI();
        this.isRolling = true;
        ...
        const checkDone = () => {
            ...
            if (completedRolls === 3) {
                this.board.time.delayedCall(1000, () => {
                    ...
                    const moveDiceToCorner = (dieObj, targetX, targetY, onCompleteCallback) => {
                        this.board.tweens.add({
                            targets: dieObj.mesh,
                            x: targetX,
                            y: targetY,
        ...
        dice1.roll((val) => { val1 = val; checkDone(); }, forcedValues?.[0]);
        dice2.roll((val) => { val2 = val; checkDone(); }, forcedValues?.[1]);
        dice3.roll((val) => { val3 = val; checkDone(); }, forcedValues?.[2]);
    }
    handleJailDiceRoll(tiles, players, forcedValues) {
        if (this.isRolling)
            return;
        this.board.hideUI();
        this.isRolling = true;
    ...
    clearDice() {
        ...
        this.board.tweens.add({
            ...
            duration: 500,
            onComplete: () => {
                const playerTokens = this.board.players.map((p) => p.token);
