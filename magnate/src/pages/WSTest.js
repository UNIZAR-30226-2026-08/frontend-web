...
export function WSTest() {
    ...
    // END OF DEBUGGING
    */
    return (_jsxs("div", { className: 'flex justify-center items-center min-h-screen bg-[url(src/assets/bg_city.jpg)] bg-cover bg-center bg-no-repeat ', children: [_jsx("div", { className: 'absolute inset-0 bg-black/60 backdrop-blur-[8px]' }), _jsxs("div", { className: 'relative flex flex-col items-center gap-6 p-3 w-full', children: [_jsx(Button, { onClick: askForGame, variant: 'magnate', className: `bg-[var(--color-primary)] text-[var(--color-text)] text-[28px] uppercase font-bold h-[60px] w-[220px]
                        ${bouncyAnimation}
                        `, children: "Pedir juego" }), _jsx(Button, { onClick: enterGame, variant: 'magnate', className: `bg-[var(--color-primary)] text-[var(--color-text)] text-[28px] uppercase font-bold h-[60px] w-[220px]
                        ${bouncyAnimation}
                        `, children: "Entrar al juego" }), _jsx(Button, { onClick: askActionThrowDices, variant: 'magnate', className: `bg-[var(--color-primary)] text-[var(--color-text)] text-[28px] uppercase font-bold h-[60px] w-[220px]
                        ${bouncyAnimation}
                        `, children: "Mandar mensaje" })] })] }));
}
